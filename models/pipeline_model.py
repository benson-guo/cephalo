...
def pipeline_model(model, pp_mesh, parallel_dims, device, loss_fn, args):
    ...
    def _build_stage(stage_idx, start_layer, stop_layer, is_first=False, is_last=False):
        ...
        layers_io_shape = (batch_size, seq_length, hidden_size)

        if is_first:
            if is_vision_model(args.model_name):
                input_shape = (batch_size, 3, args.image_size, args.image_size)
                input = torch.randn(input_shape, dtype=torch.float32, device=device)
            else:
                # For text models, input shape is (batch_size, seq_length) of ints
                input_shape = (batch_size, args.seq_length)
                input = torch.randint(0, args.vocab_size, input_shape, device=device)
        else:
            input = torch.randn(layers_io_shape, dtype=mp_dtype, device=device)

        if is_last:
            if is_vision_model(args.model_name):
                output_shape = (batch_size, seq_length, num_labels)
            else:
                # For text models, output shape is (batch_size, seq_length, vocab_size)
                output_shape = (batch_size, seq_length, args.vocab_size)

            output = torch.randn(output_shape, dtype=torch.float32, device=device)
        else:
